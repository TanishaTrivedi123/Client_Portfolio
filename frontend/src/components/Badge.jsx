    ...
    <section className='p-6 sm:p-10 bg-black w-full flex flex-col justify-center items-center' aria-label="Video editing and thumbnail design skills">
      ...
      <div className='text-primaryText mt-6 sm:mt-10 flex flex-wrap justify-center gap-2 sm:gap-4'>
        {skills.map((val, key) => (
          <span 
            key={key}
            className="px-3 sm:px-4 py-1 sm:py-1.5 font-quintessential font-medium bg-white/10 border select-none border-white/20 rounded-full text-xs sm:text-sm md:text-sm uppercase tracking-wide transition duration-300 hover:bg-orangeColor hover:text-primaryText hover:scale-105 cursor-default"
          >
            {val}
          </span>
        ))}
